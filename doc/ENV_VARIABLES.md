...
### Для управления автоповышением прав (Auto-escalation):
- `AUTO_SETUP_NO_ESCALATE` - при значении `1` отключает автоматический вызов `pkexec`/`sudo`. Скрипт сразу завершается с кодом 1 при отсутствии root-прав (аналог флага `--no-escalate`). Предназначено для CI/CD и неинтерактивных сценариев.

> [!NOTE]
> Переменные `SUDO_PASSWORD` и `AUTO_SETUP_PASSWORD` устарели (deprecated) и больше не используются, так как повышение привилегий выполняется стандартными средствами Linux (`pkexec` или `sudo`) автоматически или до запуска скрипта.
